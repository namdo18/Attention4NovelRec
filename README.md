...
`NovelNet` 한계점  
    ...
    - 이용자들의 탐색시간을 경감시켜줄 수 있으며, 실효성과 목적적합성을 충족시킬 수 있는 모델 개발 필요  
...
- 평가결과
    ...
    - 유효소비가 예상되는 새로운 소설을 추천하며, 비즈니스 실효성 및 목적합성 충족
    - 다양한 지표로 비즈니스 실효성 평가가 가능할 것 
      - e.g. 새로운 소설에 대한 유효소비 주기 변화, 추천된 새로운 소설과 이용자가 탐색한 새로운 소설 간의 유효소비 비율

- 향후 과제
    - [x] ~~cross-validation 기법을 이용하여 재학습 후 성능비교~~
        - 결과 : 소폭 성능 향상에 그침
            - 연구 :유효 인터렉션 1개 세션데이터, 희소한 소설데이터 영향으로 성능 한계 추정
                - Li et al.2022[[1](#reference)] 연구에서는 세션 길이 1개 및 등장횟수 5회 미만 소설 데이터 제거 후 진행함
            - [ ] [[1](#reference)] 조건 부합 데이터 제거 후 성능비교
